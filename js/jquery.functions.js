...
//		resizeFilesListContainer();
	...
	$('.files-list table tr').hover(function(){ $(this).addClass('hover'); }, function(){ $(this).removeClass('hover'); });
	// ------- .Наведение мышкой на строки в таблице файлов -------
	...
	$(".select-group .arrow, .select-group-text").live('click',function(){
		...
		if (parent.find(".select-group-list").is(":visible"))
			parent.find(".select-group-list").hide();
		else
			parent.find(".select-group-list").show();
...
function resHome(){
	...
	if(h>500){
		if($('.wide').size()){$('.content-wrapper-column').height(h-130);}
		else if(!$('.video-home').size()){$('.content-wrapper-column').height(h/2-78);$('.first-column').height(h-130);}
